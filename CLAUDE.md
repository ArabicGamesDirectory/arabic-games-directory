...
│   │   └── FilterSelect.tsx    # "use client" — generic <select> for filter dropdowns (country, genre, etc.); takes paramName + options + a "default" placeholder for the empty/All option; navigates on change, optionally resetting the matching pageParam
│   ├── i18n/
...
- **Form validation (submit form):** All required-field validation is done in JS inside `onSubmit` — no HTML `required` attributes. An `errors: Record<string, string>` state holds per-field messages; cleared and re-evaluated on each submit attempt. The `Field` component accepts an optional `error` prop and renders it as a small red `<p>` below the field. The `inputCls(field?)` helper returns the base input class string with a red border (`border-red-500/50 focus:ring-red-500`) when `errors[field]` is set. Required fields: name, short_description, developer, country (≥1), genres (≥1), platforms (≥1), gameplay_modes (≥1), game_engine. **New form field defaults:** all text inputs and checkboxes are empty/unchecked on a fresh submission. The only pre-selected values are the two required dropdowns: `status` defaults to `"announced"` and studio `type` defaults to `"studio"` — selects must always show an option.
- **Studio page games list:** The studio detail page (`/studios/[slug]`) fetches games via `.eq("studio_id", studio.id)` and renders them using the **exact same card structure as the homepage game cards** — `article` container with `overflow-hidden rounded-xl`, `flex flex-col sm:flex-row`, thumbnail on the start side (`w-full sm:w-[230px] shrink-0 object-cover self-stretch`, no fixed height), content in `flex-1 min-w-0 p-4`. Content: name (linked to `/games/[slug]`) + status badge row with developer name below, country · platforms · date meta line, pooled tags row (genres gray, gameplay_modes blue, monetization amber, game_engine gray/faint — capped at 5 with `+N` overflow). No description on cards. Query selects: `slug, name, developer, status, country, platforms, genres, gameplay_modes, monetization, game_engine, release_date, thumbnail_url`. If no games match, shows "No games in the directory yet."
...
- **Homepage filters:** Each tab has its own filter UI driven by URL query params; filters compose with search and sort. **Games:** platform pills (`?platform=PC|Mobile`), status pills (`?status=released|in_dev`), country dropdown (`?country=Egypt`), genre dropdown (`?genre=Action`). **Studios:** type pills (`?studioType=individual|team|studio`), country dropdown (`?studioCountry=Egypt`). **Communities:** type pills (`?communityType=online|in_person|hybrid`), topic pills (`?communityTopic=Game Development|Game Programming|Game Art|Game Design`), country dropdown (`?communityCountry=Egypt`). Tab-scoped filters do NOT carry across tab switches (only `q` does); they're erased when switching tabs because they don't apply. **Implementation:** validation done up-front against `Set`s constructed from `COUNTRY_OPTIONS`, `GENRE_VALUES`, `COMMUNITY_TOPIC_VALUES`; invalid values silently coerce to `null`. Each tab has a `buildXFilterHref(extra)` helper that preserves all current filters by default, and accepts `null` values in `extra` to remove a key (used by the "All" pill on the games tab to clear platform+status). Pagination/clear-search/search-form-hidden-inputs all preserve every filter param. **Active-filter chips:** rendered above the sort row when any filter is active — each chip shows the human-readable label (translated where applicable: country, genre, status, type, topic) and links to a URL with that one param removed. Component is a local helper `<ActiveFilterChips>` defined at the bottom of `page.tsx`. **`<FilterSelect>` (`src/components/FilterSelect.tsx`):** generic `"use client"` `<select>` wrapper for high-cardinality filters (country has 18 options, genre has 25). Adds an empty-value `<option>` at the top with a `defaultLabel` (e.g. "All countries") that maps to "no filter" — empty value = `params.delete(paramName)`. Sets pageParam to deletion on change so results reset to page 1.
