...
- **Submitter info persistence:** Both `SubmitForm` and `StudioSubmitForm` persist `submitter_name` and `submitter_email` to `localStorage` (keys `submitter_name`, `submitter_email`) on every successful submission. On mount, both fields are initialised from `localStorage` so returning submitters don't have to retype their details. The fields are fully controlled inputs (`submitterName` / `submitterEmail` state); validation and payload construction use the state values directly rather than reading from `FormData`.
