...
- **Admin flow:** Admin signs in with Supabase email/password auth → page loads both pending game and studio submissions → a "Games / Studios" tab switcher at the top of the queue shows counts. Both game and studio cards use the same expandable pattern: compact header always visible, "View details ↓" toggle reveals a `DetailRow`-based detail section. Update submissions (those with `game_id` / `studio_id` set) highlight changed fields in amber with a "changed" badge and "was: [old value]" annotation. The admin page batch-fetches original games and original studios at load time (stored in `originalGames` and `originalStudios` maps keyed by id) so diffs are available immediately. Studio update cards include a "View current studio ↗" link. Approve/Reject call server-side API routes that verify the session cookie + admin email, then use the service-role client for DB writes. If `getUser()` returns an auth error (e.g. stale refresh token), the page calls `signOut()` to clear bad cookies and shows the login form cleanly.
...
- **Submitted by (public display):** Both `games` and `studios` tables have a `submitted_by text` column (nullable). It is populated with `submitter_name` from the submission when a **new** entry is approved (INSERT path in `/api/approve` and `/api/approve-studio`). It is **not** overwritten on update approvals, preserving the original submitter credit. The game detail and studio detail pages display `"Submitted by: {name}"` in small muted text below the description when the field is set. Only the name is shown publicly — email is never exposed.
