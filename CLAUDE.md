...
| status | game_status enum | announced / in_dev / prototype / early_access / released / on_hold / cancelled |
...
19. Thumbnails via Supabase Storage (deferred — keeping text-only for now)
20. Email notification to submitter on approve/reject
21. Charts on stats page instead of plain lists
22. Link games to studios via `studio_id` FK (currently stores studio name as plain text in `developer`; name-matching is done with `.ilike` at render time)
