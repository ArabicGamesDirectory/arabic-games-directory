...
- **Game detail page layout:** `src/app/[locale]/games/[slug]/page.tsx` uses a three-section layout. (1) **Hero block** — full-width `aspect-[460/215]` thumbnail (when present), then title `h1`, developer name as a separate `<p>` (linked to `/studios/[slug]` when `studio_id` is set), status badge + country + platform + release date as a single `flex flex-wrap gap-2 mt-3` pill row, description with `whitespace-pre-wrap`. (2) **Two-column details grid** — `grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8`; each cell uses a `DetailCell` helper (small-caps label + content below); Genres/Gameplay Modes/Monetization use pill `<Tag>` badges; Game Engine is plain `text-sm text-c-text`; empty cells are omitted. Publishing is **not** in the grid — it appears inline in the Links section header row as `LINKS · Self-published` (or publisher name) using `text-xs text-c-faint`. (3) **Links section** — header row is `flex items-center gap-2` with the small-caps "Links" label and optional publishing annotation beside it; pills are `inline-flex` buttons linking directly to external URLs (unlike the homepage card which links to the game detail page — on the detail page itself, links always go external). (4) **"Submitted by"** — absolute last element on the page, `mt-8 text-xs text-c-faint`, below the links section. The "Back to directory" link and "Suggest an update" button stay in a `flex justify-between` row at the very top.
