...
- **Game–studio join:** All game queries that need to link to a studio use Supabase's FK join syntax: `.select("*, studios(slug)")`. This returns `studios: { slug: string } | null` on each game row. No runtime name-matching at render — `g.studios?.slug` is used directly. The homepage no longer builds a `studioSlugMap`.
