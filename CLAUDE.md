...
16. ~~Genres as checkboxes~~ ✓ Done (17 predefined genres: Action, Adventure, Puzzle, RPG, Strategy, Simulation, Sports, Racing, Horror, Platformer, Shooter, Fighting, Survival, Visual Novel, Educational, Idle, Other; stored as English values in `text[]`; translated via `genres` i18n namespace; pre-fills on update form)
17. ~~Form validation feedback~~ ✓ Done (`errors` state validated on submit; inline red error messages per field via `Field` component `error` prop; `inputCls(field?)` helper applies red border when field has error; validates: name, description, country ≥1, genres ≥1, platforms ≥1; HTML `required` removed — all validation through JS)
...
- **Genres:** Controlled list of 17 genres defined inside `SubmitForm` as `GENRE_OPTIONS` (value = English string stored in DB, key = camelCase i18n key in the `genres` namespace). Stored as `text[]` in `games.genres`. The submit form renders them as a `CheckboxGroup` — same component used for platforms/gameplay modes/monetization. Pre-fills on update form by matching `initialData.genres` values. Do not change the English `value` strings without a data migration.
