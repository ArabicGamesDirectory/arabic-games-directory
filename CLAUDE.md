...
- **Homepage submit CTA:** A single `<SubmitMenu>` (client) renders a "Submit ▾" indigo button in the header that opens a dropdown listing the three submission targets (game, studio, community). Closes on outside click + Escape. This replaced the prior 3-button row which got crowded — the dropdown stays one element wide regardless of viewport. The button label uses `tCommon("submit")`; the menu items reuse the existing `submitGame` / `submitStudio` / `submitCommunity` keys.
