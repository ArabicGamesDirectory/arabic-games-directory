...
├── src/
│   ├── app/
│   │   ├── layout.tsx          # Root layout — reads locale via getLocale(), sets <html lang dir>; defines metadataBase + title template + default OG/Twitter
│   │   ├── not-found.tsx       # Global 404 page (root-level fallback for truly unmatched routes); bilingual EN+AR stacked since it renders outside the NextIntlClientProvider tree
│   │   ├── robots.ts           # robots.txt generator — disallows /admin, /api/, update routes; references /sitemap.xml
│   │   ├── sitemap.ts          # sitemap.xml generator — static routes + every game/studio/community slug, with hreflang alternates per locale
│   │   ├── feed.xml/
│   │   │   └── route.ts        # GET — RSS 2.0 feed of the 30 most recently approved games. Content-Type application/rss+xml; 5-min edge cache. English-only (most readers can't render Arabic titles cleanly). Discoverable via root-layout `<link rel="alternate">`.
...
│   │   │   ├── not-found.tsx   # Locale-scoped 404 — catches notFound() calls from update/[slug] etc., uses i18n + locale layout (footer visible)
...
│   │   │   ├── about/
│   │   │   │   └── page.tsx    # Static About / FAQ page — mission, scope, how to submit, moderation, FAQ
│   │   │   ├── update-community/
│   │   │   │   └── [slug]/
...
│   │   ├── useDraftState.ts    # "use client" — useState replacement that persists to localStorage under a per-field key, so SubmitForm survives an accidental tab close. Drop-in API. Pass `enabled: false` to fall back to plain useState (used during the update flow so initialData wins). Sibling `clearDraft(prefix)` wipes every key under a prefix in one shot — called after successful submit.
│   │   ├── Skeleton.tsx        # Tailwind-only skeleton primitive — animated `bg-c-tag` block. Used by the per-route `loading.tsx` files (homepage, game/studio/community detail, stats) so first-paint shows the page shape while server data fetches.
│   │   ├── FilterPill.tsx      # Server component — Link-styled-as-pill that navigates to the homepage with a single filter param applied (e.g. clicking "Action" → "/?genre=Action"). Used on game cards and game/studio detail pages so visual tags double as filter shortcuts. Caller controls className (color/shape) — the component only adds hover affordance + the locale-aware Link href. Studio/community pills aren't using it yet (different param names per tab — would need a `mode` or different component).
│   │   ├── TopNav.tsx          # Sticky site-wide top navigation rendered from `[locale]/layout.tsx`. Brand link on start, Stats / About / Submit▾ + LanguageSwitcher + ThemeToggle on end. `flex-wrap` so the link cluster wraps below the brand on narrow screens. Uses `nav.*` i18n namespace for brand label. Replaced the previous floating bottom-corner Theme + Language toggles which overlapped page content on mobile.
│   │   ├── Disclaimer.tsx      # Server component — site-wide footer with the curation/volunteer disclaimer + About + Contact links (i18n-driven via the `footer` namespace); rendered from `[locale]/layout.tsx` so it appears on every page
...
│       ├── genres.ts           # GENRE_VALUES (26 entries) + GENRE_I18N_KEYS map + normalizeGenres() helper — single source of truth shared by SubmitForm, homepage genre filter, and /api/approve
...
│       ├── formatDate.ts       # formatDate(iso, locale) — wraps `Intl.DateTimeFormat(locale, { dateStyle: "medium" })`; returns null for empty/invalid input. Used on game/studio/community detail pages for the "Last updated · Added" footer
│       └── alternates.ts       # languageAlternates(path) → builds the `alternates.languages` block (en + ar + x-default) for Next.js metadata so each page emits `<link rel="alternate" hreflang>` tags telling Google about its translations
...
SITE_URL=https://arabicgames.directory  # Server-only — optional. Used in (1) the absolute admin URL embedded in the submission webhook so clicking the Discord embed title opens /admin?tab=..., (2) `metadataBase` in `app/layout.tsx` so OG/Twitter card images resolve to absolute URLs, (3) `robots.ts` host + sitemap base, (4) every URL in `sitemap.xml`. Defaults to `https://arabicgames.directory` if unset, so dev environments still build but social previews + sitemap will point at production.
```
...
48. ~~Submit form autosave~~ ✓ Done (new `src/components/useDraftState.ts` exports `useDraftState(key, initial, enabled)` — a useState replacement that persists each state slice to localStorage under a per-field key, restoring on mount. Wired into `<SubmitForm>` for the high-typing-cost fields: name, short_description, website_url, game_engine, publisher_name, publishing_type, status, release_date, genre_other_checked, genre_other_text. Those uncontrolled `defaultValue` inputs were converted to controlled `value`+`onChange`. Autosave is disabled during the update flow (`enabled: !isUpdate`) so initialData wins. After successful submit, `clearDraft(DRAFT_PREFIX)` wipes every key under `submit-game:` so the next visit starts blank. Storage keys live under the `submit-game:` prefix. Checkbox fields (country/genres/platforms/gameplay_modes/monetization), multi-developer tags, and thumbnail uploads are NOT autosaved — they're either fast to re-click or hold non-serializable state. Apply the same hook to StudioSubmitForm + CommunitySubmitForm later if needed.)

47. ~~Loading skeletons on key routes~~ ✓ Done (new `src/components/Skeleton.tsx` primitive — animated `bg-c-tag` block. Five `loading.tsx` files added: `[locale]/loading.tsx` (homepage with header + tabs + filters + 4 card skeletons), `[locale]/games/[slug]/loading.tsx` (hero + title + pills + details grid), `[locale]/studios/[slug]/loading.tsx` (hero + name + games-list skeleton), `[locale]/communities/[slug]/loading.tsx` (hero + topics + social links), `[locale]/stats/loading.tsx` (counter row + 8-card chart grid + full-width row — important because stats runs 4 parallel Supabase queries and is the slowest cold-visit page). Each skeleton mirrors its real page's layout shape so the first-paint hand-off is visually stable.)

46. ~~JSON-LD structured data on detail pages~~ ✓ Done (game detail emits `<script type="application/ld+json">` with schema.org `VideoGame` — name, description, url, inLanguage, image (when thumbnail set), datePublished (when status allows release date and one is set), gamePlatform, genre, author array of `Organization` (each developer; URL to studio page when a `game_studios` join match exists), publisher when `publishing_type === "with_publisher"`. Studio detail uses `Organization` schema with name/url/description/logo/image/sameAs (website). Community detail uses `Organization` too with the website + every social_links URL aggregated into `sameAs`. The blob is rendered just inside `<main>` via `dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}` — NOT via Next.js metadata, since metadata has no JSON-LD field. `siteUrl` resolves from `SITE_URL` env var with the production fallback.)

45. ~~RSS feed for new approvals~~ ✓ Done (new `src/app/feed.xml/route.ts` returns RSS 2.0 XML with the 30 most recent approved games — title, link to /en/games/[slug], description = short_description, pubDate from created_at in RFC 822 format, optional `<enclosure>` for thumbnail. Content-Type `application/rss+xml; charset=utf-8`. `Cache-Control: public, s-maxage=300, stale-while-revalidate=600` for edge caching. English-only (most RSS readers can't render Arabic in titles cleanly + URLs point at /en/ pages). Auto-discovery via `<link rel="alternate" type="application/rss+xml">` in the root layout. Footer "RSS feed →" link added to `<Disclaimer>` next to About + Contact. New `footer.rss` i18n key in en + ar.)

44. ~~In-page hreflang alternates~~ ✓ Done (new helper `src/lib/alternates.ts` exports `languageAlternates(path)` which returns the `alternates.languages` block for Next.js metadata — `{ en: "/en/foo", ar: "/ar/foo", "x-default": "/en/foo" }`. Wired into `generateMetadata` on every public page: homepage, stats, contact, about, submit, submit-community, submit-studio, and the three detail pages. Pass the path WITHOUT the locale prefix (e.g. `/games/${slug}`) — the helper builds full per-locale paths. Sitemap was already declaring alternates; this puts them on every page so search engines see them at crawl time too. `metadataBase` from the root layout makes the relative paths resolve to absolute URLs in the rendered `<link rel="alternate">` tags.)

43. ~~Stats expansion: by engine + by monetization + submissions over time~~ ✓ Done (three new charts wired into the existing stats grid. **By Engine** + **By Monetization** are horizontal bar cards alongside the existing Country/Status/Platform/Genre 2-col grid — Engine aggregates the free-text `game_engine` field (trimmed, empty skipped), Monetization aggregates the `monetization` text[] array. **Submissions over time** is a separate full-width `<VBarChart>` row below the grid, bucketing `games.created_at` by `YYYY-MM` and gap-filling silent months across `min..max` so the line of bars stays continuous. Labels are locale-aware via `Intl.DateTimeFormat(locale, { month: "short", year: "numeric" })`. New i18n keys: `stats.byEngine`, `stats.byMonetization`, `stats.submissionsOverTime` in en + ar. Same `<StatCard>` small-data fallback applies — sub-3-month directories show a `SummaryList` instead of an empty chart.)

42. ~~Site-wide top nav + floating buttons fix~~ ✓ Done (new `<TopNav>` (`src/components/TopNav.tsx`) renders on every locale page from `[locale]/layout.tsx` — sticky at top with `backdrop-blur-sm bg-c-bg/80`. Contents: brand link → home, Stats, About, Submit▾, LanguageSwitcher, ThemeToggle. `<ThemeToggle>` and `<LanguageSwitcher>` had their `fixed bottom-4 ...` styling stripped — they now render inline inside TopNav. `<SubmitMenu>` was refactored to be self-contained: it pulls labels via `useTranslations("common")` instead of taking them as props, so it can be dropped into any client tree. Homepage header was slimmed down to just `h1` + tagline (the duplicate SubmitMenu and Stats link came out — TopNav handles both). New `nav.*` i18n namespace with `brand` + `about` keys.)

41. ~~Related games on detail page~~ ✓ Done (game detail page (`/games/[slug]`) now runs two parallel Supabase queries after fetching the main game — one for "More from {primary developer}" via `.contains("developers", [primaryDev])` and one for "Similar games" via `.contains("genres", [primaryGenre]).overlaps("country", game.country)`. Both `.neq("id", game.id)` and limit to 4. Similar games dedupe against the dev set so a game by the same dev doesn't appear in both. Renders as two horizontal `<RelatedStrip>` sections (180px cards, thumbnail or TitleCover + name + status badge, mirrors homepage Recently Added shape). Either section may render alone if the other returns empty; both sections skipped entirely when both are empty. Lives between the Links section and the timestamp footer. New `gameDetail.moreFromDev` (with `{name}` placeholder) and `gameDetail.similarGames` i18n keys.)

40. ~~Branded 404 page~~ ✓ Done (two not-found.tsx files: root `app/not-found.tsx` is a bilingual EN+AR fallback for truly unmatched URLs — it renders outside the NextIntlClientProvider tree so it has its own `<html>`/`<body>` and uses hard-coded text; locale-scoped `app/[locale]/not-found.tsx` catches explicit `notFound()` calls from inside locale routes (update pages with bad slugs, etc.) and inherits the locale layout including the `<Disclaimer>` footer. New `notFound.title` / `notFound.body` / `notFound.cta` i18n keys.)

38. ~~Recently added strip on homepage~~ ✓ Done (when on the games tab with no search/filter active and on page 1, a "Recently added" horizontal strip renders above the search bar showing the 5 most recently created games (`order("created_at", desc).limit(5)`). Each card is 180px wide, links to `/games/[slug]`, shows thumbnail/TitleCover + name (truncated, `dir="auto"`) + status badge. Strip scrolls horizontally on mobile via `overflow-x-auto`. Render also gated on `recentGames.length >= 3` so a near-empty directory doesn't show a sad-looking strip. Fetch is conditional — skipped entirely (no network roundtrip) when filters are active or paging beyond page 1.)

37. ~~Last-updated/added timestamps on detail pages~~ ✓ Done (game/studio/community detail pages all show a small `text-xs text-c-faint` footer at the bottom: `Last updated: 4 May 2026 · Added: 2 Apr 2026`. Pulls `updated_at` and `created_at` from the row (already in the schema); both fields are independently optional so legacy rows missing one still render the other. Locale-aware via new `src/lib/formatDate.ts` (`Intl.DateTimeFormat(locale, { dateStyle: "medium" })`) — Arabic renders eastern-Arabic numerals + Arabic month names. New `common.lastUpdated` + `common.addedOn` i18n keys.)

36. ~~Clickable filter tags on cards & detail pages~~ ✓ Done (new `<FilterPill>` (`src/components/FilterPill.tsx`) wraps a tag in a locale-aware `<Link href="/?param=value">` so visual pills double as filter shortcuts. Wired into the homepage game cards (status badge + genre tags), the game detail page hero (status / country / platform pills) + the genre tags inside the details grid, and the studio detail page games list (status badge + genre tags). Other tag types — `gameplay_modes`, `monetization`, `game_engine` — stay as plain text since the homepage has no filter for them. Studio cards on the homepage and community cards still show plain pills because their filter params (`studioCountry`, `communityCountry`, etc.) differ from the games tab — punting until needed.)

35. ~~About / FAQ page + footer link~~ ✓ Done (new `/[locale]/about/page.tsx` + `about.*` i18n namespace covering Mission, Scope, How to submit, How approval works, Free and open, FAQ. Inline links to /submit and /contact use next-intl `t.rich()` with `<submit>` / `<contact>` pseudo-tags in the translation strings — ICU-style placeholders won't work for clickable inline content. The site-wide `<Disclaimer>` footer now renders both an "About →" and a "Contact us →" link side-by-side instead of just contact.)

34. ~~SEO foundation: per-page metadata + sitemap + robots~~ ✓ Done (root layout now sets `metadataBase` (from `SITE_URL` env var, fallback `https://arabicgames.directory`) + `title.template = "%s — Arabic Games Directory"` + default OG/Twitter card; every public page has a `generateMetadata` that returns its own title and description. Detail pages (`/games/[slug]`, `/studios/[slug]`, `/communities/[slug]`) also expose `openGraph.images` / `twitter.images` from the entity's `thumbnail_url` so social embeds preview correctly. Update routes (`/update/[slug]` etc.) and `/admin` set `robots: { index: false }` (admin via a tiny `[locale]/admin/layout.tsx` since the page is `"use client"` and can't export metadata directly). New `src/app/robots.ts` + `src/app/sitemap.ts` (Next.js 16 conventions) — sitemap fetches all approved game/studio/community slugs and includes `xhtml:link rel="alternate" hreflang="..."` per locale so Google understands `/en/games/foo` and `/ar/games/foo` are translations of each other. Mobile fix bundled in: the homepage tabs/Stats row had `flex items-center justify-between` with no wrap, so the Stats link ran into the tabs at narrow widths — added `gap-2 flex-wrap` so Stats wraps to its own line below 640px.)

33. ~~Stats page expansion: counter row + top studios + by release year~~ ✓ Done (added a 3-card headline counter row above the chart grid showing Games / Studios / Communities with each card linking to the matching homepage tab; added a Top Studios leaderboard ranking the 10 studios with the most linked games — each row links to `/studios/[slug]` — populated by counting `game_studios` join rows in JS after a single `select("studio_id")` fetch; added a By Release Year vertical bar chart (`<VBarChart>` in `src/components/StatsCharts.tsx`) with year-range gap-filling so silent years render as zero-bars, gated on `statusAllowsReleaseDate(status)` AND non-null `release_date` and sanity-bounded to `1970..currentYear+5`; the previous "{count} games in the directory" subtitle was removed since the Games counter shows the same number more prominently. Stats page now runs four parallel queries instead of one — see Stats charts convention for details.)
...
- **Genres:** 26 predefined genres tuned for the MENA gaming context. Educational is a first-class genre (not under "Other") because Arabic educational and Islamic apps are prominent in the region. Visual Novel is separate from Adventure because several MENA studios build story-driven games rooted in regional folklore. Casual and Idle are included as top-level genres because mobile dominates the region. Family and Made for Kids are included as distinct genres — Family targets mixed-age audiences, Made for Kids targets children specifically. Dress up is included as a first-class genre — it's a popular standalone category in MENA mobile gaming (especially for kids/casual audiences). Defined outside the component as `GENRE_BASE_VALUES` (string array, used for state init) and inside the component as `GENRE_OPTIONS_BASE` (with translated labels via `tGenres`). "Other" is rendered as a standalone toggle below the `CheckboxGroup`; when checked it reveals a free-text input (`genreOtherText` state). On submit, the combined array (base checkboxes + Other text when checked) is run through `normalizeGenres()` (`src/lib/genres.ts`) which: trims each entry, drops empties and the literal string `"Other"`, case-insensitively folds matches to canonical (`"card game"` → `"Card / Board Game"`, `"PUZZLE"` → `"Puzzle"`), and dedups case-insensitively. The same helper runs again in `/api/approve` as defense in depth so legacy pending submissions queued before this normalization also get cleaned on approve. Pre-fills on update: known values go to `CheckboxGroup` initialValues; any unrecognized value sets `genreOtherChecked=true` and `genreOtherText` to that value. Stored as `text[]` in `games.genres`. Do not change the English `value` strings without a data migration. **Legacy values** (`"Shooter"`, `"Idle"`, `"Survival"`) may still exist in the DB from before the rename — they display as raw strings on game pages (no translation lookup needed).
...
- **Stats charts:** The stats page (`/stats`) is a server component that runs four parallel Supabase queries (games, communities count head-only, studios id+slug+name, game_studios join rows), aggregates counts, resolves translated labels, then passes `ChartEntry[]` arrays + a counter object + a `StudioRank[]` to the `StatsCharts` client component. All i18n is resolved server-side. Layout: a `<CounterRow>` of 3 clickable cards (Games / Studios / Communities — each links to the matching homepage tab) sits above a 2-column grid containing eight `<Card>` / `<StatCard>` panels: By Country (HBar) · By Status (Donut) · By Platform (HBar — was a donut, converted because 13+ unordered platform names made the donut legend unreadable) · By Genre (HBar) · By Engine (HBar — aggregates the free-text `game_engine` field; trimmed + skipped if empty) · By Monetization (HBar — aggregates the `monetization` array, multi-value per game) · Top studios (ranked list, top 10 by `game_studios` count, descending; ties broken by name; each row links to `/studios/[slug]`) · By Release Year (`<VBarChart>` — vertical bar chart, gap-filled across `min..max` year so silent years render as zero-bars; only includes games whose status is in `STATUSES_WITH_RELEASE_DATE` AND has non-null `release_date`; sanity bound rejects years before 1970 or beyond `currentYear + 5`). Below that grid, a single full-width `<StatCard>` renders **Submissions over time** as a `<VBarChart>` — buckets `created_at` by `YYYY-MM`, gap-fills across the min→max month so silent months render as zero-bars; bar labels are locale-aware short month + year via `Intl.DateTimeFormat(locale, { month: "short", year: "numeric" })` (renders `"May 2026"` in en, `"مايو 2026"` in ar). The Status donut renders count alongside name in the legend (e.g. `Released (152)`) via a `Legend` `formatter` that reads `entry.payload.value` — needed so tiny slivers like `Cancelled` or `Announced` are still legible. Tooltips use `var(--c-surface)` / `var(--c-border)` / `var(--c-text)` inline styles so they theme-switch correctly. **Small-data fallback:** each `StatCard` independently checks data length — `length === 0` shows muted "No data yet"; `length < 3` renders `SummaryList` (`flex justify-between` rows with label + count badge); otherwise the chart. Top Studios uses a separate `<Card>` wrapper (no fallback) because its data shape is `StudioRank` (not `ChartEntry`) and it has its own empty state (`noStudios` label). Requires `recharts` (already in `package.json`). The previous `subtitleSingular` / `subtitlePlural` count line under the page title was removed when CounterRow landed (redundant — the Games counter shows the same number more prominently); the i18n keys are kept in `messages/{en,ar}.json` for now in case they're reused, just not referenced by the page.
